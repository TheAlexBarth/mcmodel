...
mcmc_run_internal = function(
   ...
   #region \- load params -----------------------
    ...
    max_save_size = (n_iter-n_burn)/thin
    save = list()
    ...
    for(param in save_names) {
        ...
        save[[param]] = array(NA, dim = c(max_save_size, param_dims))
        if(burn_adapt) run_save[[param]] = array(NA, dim = c(min_burn+1, param_dims))
    }
    #endregion---------------------------------------------
...
mcmc_run_internal_alt = function(
    data_list,
    const_list,
    init_list,
    prior_pars,
...
z                } else {
    ...
    # reformatting to ideal structure
    for(param in save_names) {
        d = pd[[param]]
        ns = nrow(save[[param]])

        if(is.null(d)) {
            save[[param]] = as.numeric(save[[param]][, plen[[param]]])
        } else {
            arr = array(save[[param]], dim = c(ns, d))
        }
    }
