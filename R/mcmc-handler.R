...
mcmc_run_internal = function(
    ...
    if(adaptive) {
        if(!all(sapply(c('min_burn', 'max_iter','goal_ess'), exists))) cat('\n', 'Running on default chain settings....')
        if(!any(sapply(c('burnin', 'n_iter'), function(x) !is.null(x)))) warning('Provided non-adaptive args but running on adaptive = TRUE')

        out = adaptive_mcmc_run(
            data_list = data_list,
            const_list = const_list,
            init_list = init_list,
            prior_pars = prior_pars,
        ...
        if(!all(sapply(c('burnin', 'n_iter'), exists))) cat('\n', 'Running on default chain settings....')
        if(!any(sapply(c('min_burn', 'max_iter','goal_ess'), function(x) !is.null(x)))) warning( 'Provided adaptive formals but ran on adaptive = FALSE')
